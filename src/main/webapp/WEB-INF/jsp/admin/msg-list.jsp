		...
		<table class="layui-table" style="width: 90%; margin: 0 auto;">
		  ...
		  <tbody>
		  	<c:forEach items="${page.records }" var="row" varStatus="vs">
		  		<tr>
		  			<td>${vs.count }</td>
		  			<td><fmt:formatDate value="${row.msgTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
		  			<td>
		  				<c:if test="${row.isRead == -1 }">
		  					<span class="layui-badge">
				  				<c:if test='${row.msgType == "NEW_REGIST_ENT" }'>征集人待审核</c:if>
				  				<c:if test='${row.msgType == "NEW_REGIST_SUP" }'>供应商待审核</c:if>
				  				<c:if test='${row.msgType == "NEW_TENDER" }'>征集申请待审核</c:if>
			  				</span>
		  				</c:if>
		  				<c:if test="${row.isRead != -1 }">
		  					<c:if test='${row.msgType == "NEW_REGIST_ENT" }'>征集人待审核</c:if>
		  			...
		  			<td>
		  				...
		  				<c:if test="${row.isRead == -1 }">
		  					<a class="layui-btn layui-btn-sm" href="${row.alink }"><i class="layui-icon">&#xe702;</i>前往处理</a>
		  				</c:if>
