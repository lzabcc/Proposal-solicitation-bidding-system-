  	...
  	<div class="layui-body">
	    ...
	    <div class="bread-nav">
	    	<span class="layui-breadcrumb">
			  ...
			  <a href="/ssm/admin/supmgr">供应商管理</a>
			  <a href="#">激活账号</a>
			</span>
	    </div>
	    <div class="layui-bg-green pd-5"><h3>待激供应商账号</h3></div>
	    <div class="search-form layui-row">
			<form action="/ssm/admin/entmgr" method="post">
		  ...
		  <tbody>
		  	<c:forEach items="${page.records }" var="row">
		  		<tr>
		  			...
		  			<td>
		  				<a class="layui-btn layui-btn-sm" href="/ssm/admin/ent-detail?tid=${row.supplierId }"><i class="layui-icon">&#xe702;</i>查看</a>
		  				<c:if test="${row.status eq -1 }">
		  				<a class="layui-btn layui-btn-sm layui-btn-normal" href="javascript:void(0);" onclick="act(${row.supplierId });"><i class="layui-icon">&#xe605;</i>激活</a>
		  				</c:if>
		  				<a class="layui-btn layui-btn-sm layui-btn-danger" href="javascript:void(0);" onclick="cfm(${row.supplierId });"><i class="layui-icon">&#xe640;</i>删除</a>
		  			</td>
		  		</tr>
		  	</c:forEach>
		  </tbody>
