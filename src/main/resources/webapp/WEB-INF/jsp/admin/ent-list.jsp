...
<div class="layui-layout layui-layout-admin">
		...
		<table class="layui-table">
		  ...
		  <tbody>
		  	<c:forEach items="${page.records }" var="row">
		  		<tr>
		  			<td class="text-center">${row.enterpriseName }</td>
		  			<%-- <td>${row.tel }</td>
		  			<td>${row.email }</td> --%>
		  			...
		  			<td class="text-center">
		  				<a class="layui-btn layui-btn-xs" href="/ssm/admin/ent-detail?tid=${row.entId }"><i class="layui-icon">&#xe702;</i>查看详情</a>
	...
	<script type="text/javascript">
		...
		function cfm(eid){
			layer.confirm('确认删除征集单位吗？', {icon: 3, title:'提示'}, function(index){
				window.location.href="/ssm/admin/deleteent?eid="+eid;
			});
		}
		function act(eid){
			layer.confirm('确认激活该征集单位吗？', {icon: 3, title:'提示'}, function(index){
				window.location.href="/ssm/admin/actent?eid="+eid;
			});
		}
		function dis(eid){
			layer.confirm('确认停用该征集单位？', {icon: 3, title:'提示'}, function(index){
